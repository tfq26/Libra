    ...
    <div class="flex w-full max-w-screen-lg flex-col items-center justify-center text-center">
      ...
      >
        ...
        <img
          :src="lightLogo"
          alt="Libra Logo"
          class="mx-auto h-36 w-auto my-8 dark:hidden"
        />

        <!-- Dark Mode Logo: Hidden by default, visible (as a block) in dark mode -->
        <img
          :src="darkLogo"
          alt="Libra Logo"
          class="mx-auto h-36 w-auto my-8 hidden dark:block"
        />
      <transition
        appear
...
// If there's a persisted error in sessionStorage, use it to populate the error prop
try {
  if (typeof sessionStorage !== 'undefined') {
    const raw = sessionStorage.getItem('libra:lastError');
    if (raw) {
      const parsed = JSON.parse(raw);
      if (parsed) {
        // Replace the error prop with an object containing full details when in dev mode
        if (!props.error && parsed) {
          props.error = parsed;
        }
      }
    }
